...
import { certificationTable } from "@/db/schema/certifications";
      ...
      try {
        ...
        await db.transaction(async (trx) => {
          ...
          if (certifications && Array.isArray(certifications)) {
            const existingCertifications = await trx
              .select()
              .from(projectTable)
              .where(eq(projectTable.docId, existingDocument.id));

            const existingCertificationsMap = new Set(
              existingCertifications.map((proj) => proj.id)
            );

            for (const proj of certifications) {
              const { id, ...data } = proj;
              if (id !== undefined && existingCertificationsMap.has(id)) {
                await trx
                  .update(certificationTable)
                  .set(data)
                  .where(
                    and(
                      eq(certificationTable.docId, existingDocument.id),
                      eq(certificationTable.id, id)
                    )
                  );
              } else {
                await trx.insert(certificationTable).values({
                  docId: existingDocument.id,
                  ...data,
                });
              }
